...
export default function OurProcess() {
  ...
  return (
    <section
      className={`relative ${themeClasses.sectionBg} py-20 md:py-25 overflow-hidden`}
    >
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div
          className="absolute top-1/3 right-20 w-96 h-96 bg-[#003144]/10 rounded-full blur-3xl animate-pulse"
          style={{ animationDelay: "1s" }}
        />
        <div
          className="absolute bottom-0 left-1/3 w-80 h-80 bg-white/5 rounded-full blur-3xl animate-pulse"
        ...
        <div className="absolute top-1/4 right-1/4 w-40 h-40 border-2 border-white/10 rounded-full" />
      ...
      <div className="container mx-auto px-4 md:px-8 relative z-10">
        ...
        <div className="relative">
          <div className="hidden md:block absolute top-1/3 left-0 right-0 h-1 bg-gradient-to-r from-white/40 via-white/60 to-white/40 -z-10 md:top-24" />
              ...
              >
                  ...
                  className={`relative bg-white/95 backdrop-blur-sm border-2 rounded-2xl p-6 md:p-8 transition-all duration-300 cursor-pointer
                    ${
                      activeStep === step.number
                        ? "border-white shadow-2xl shadow-black/30 -translate-y-2 bg-white"
                        : activeStep === null
                        ? "border-white/30 hover:border-white/60 shadow-xl shadow-black/20"
                        : index === activeStep - 1 || index === activeStep + 1
                        ? "border-white/30 shadow-lg shadow-black/15"
                        : "border-white/20 shadow-lg shadow-black/15 opacity-90"
                    }
                  `}
                >
                  {/* Number Badge */}
                  <div
                    className={`inline-flex items-center justify-center w-14 h-14 rounded-full font-bold text-lg mb-6 transition-all duration-300
                      ${
                        activeStep === step.number
                          ? "bg-gradient-to-br from-[#009f8b] to-[#008575] text-white scale-110 shadow-lg"
                          : "bg-gradient-to-br from-[#009f8b]/20 to-[#003144]/20 text-[#009f8b]"
                      }
                    `}
                  >
                    {step.number}
                ...
                {index < processSteps.length - 1 && (
                  <div className="md:hidden flex items-center justify-center h-8">
                    <div className="w-0.5 h-full bg-gradient-to-b from-white/40 to-white/20" />
                  </div>
                )}
              </div>
            ))}
        ...
        <div className="mt-16 md:mt-20 text-center">
          ...
          <button className="inline-flex items-center cursor-pointer justify-center px-8 py-3 bg-white text-[#009f8b] font-semibold rounded-full hover:shadow-2xl hover:shadow-black/40 hover:-translate-y-1 transition-all duration-300 hover:bg-white/95">
            Let's Work Together
